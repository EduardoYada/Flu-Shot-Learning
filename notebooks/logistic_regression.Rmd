...
```{r}
# preProcess_missingdata_model <- preProcess(as.data.frame(trainData), method='knnImpute')
...
## Dropping all rows with any NA

```{r}
form_seasonal = as.formula(paste("seasonal_vaccine", paste(features, collapse=" + "), sep=" ~ "))
model_seasonal <- glm(form_seasonal, data = trainData, family = "binomial")

form_h1n1 = as.formula(paste("h1n1_vaccine", paste(features, collapse=" + "), sep=" ~ "))
model_h1n1 <- glm(form_h1n1, data = trainData, family = "binomial")
...
```{r}
evaluate_model = function(model, data, target, plot=F){
...
add_performance = function(df, model, version, target){
  df[nrow(df) + 1, ] = c(target,
                         version,
                         "train",
                         evaluate_model(model, trainData, target))
  
  df[nrow(df) + 1, ] = c(target,
                         version,
                         "test",
                         evaluate_model(model, testData, target))
  return(df)
}
